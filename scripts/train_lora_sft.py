...
def parse_args():
    ...
    parser.add_argument(
        ...
        choices=("steps", "epoch"),
        default="steps",
        help="checkpoint 保存策略；默认按步保存，降低长上下文训练的故障回退距离。",
    )
    ...
    parser.add_argument(
        ...
        default="auto",
        help="checkpoint 路径、auto（默认，选择输出目录内最新完整 checkpoint）或 none。",
    )
...
def _checkpoint_step(path):
    """Return the numeric Trainer checkpoint suffix, or ``None``."""
    prefix = "checkpoint-"
...
def _is_resumable_checkpoint(path):
    """Require all state needed for an exact Trainer resume.

    A model-only directory is useful for inference but cannot restore optimizer,
    scheduler, dataloader position and RNG state. Incomplete directories are ignored
    so a node failure during checkpoint writing falls back to the previous receipt.
    """
    path = Path(path)
    if not path.is_dir():
        return False
    if any(not (path / name).is_file() for name in CHECKPOINT_STATE_FILES):
        return False
    if not any(path.glob("rng_state*.pth")):
        return False
    if not any((path / name).is_file() for name in CHECKPOINT_MODEL_FILES):
        return False
    try:
    ...
    return step is not None and state.get("global_step") == step


def _resolve_resume_checkpoint(output, requested):
    """Resolve explicit/automatic resume requests before loading the model."""
    if requested is None or str(requested).strip().lower() in {"", "none", "false"}:
        return None
    if str(requested).strip().lower() != "auto":
        checkpoint = Path(requested).expanduser().resolve()
        if not _is_resumable_checkpoint(checkpoint):
            raise SystemExit(f"checkpoint 不完整或不可恢复：{checkpoint}")
    ...
    for _, checkpoint in candidates:
        if _is_resumable_checkpoint(checkpoint):
...
def _torch_dataset(examples, torch):
    # Tensorize once. The previous implementation rebuilt three long tensors in
    # every __getitem__ call and repeated that work in every epoch.
    tensor_examples = tuple(
        {
    ...
    class TokenizedDataset(torch.utils.data.Dataset):
        def __len__(self):
            return len(tensor_examples)

        def __getitem__(self, index):
            return tensor_examples[index]
...
def _collate(batch, pad_token_id, torch):
    ...
    if len(batch) == 1:
        # The production configuration uses batch=1. unsqueeze returns a view and
        # avoids copying a 24K-token sample into freshly allocated padding tensors.
        return {name: value.unsqueeze(0) for name, value in batch[0].items()}
...
def main():
    ...
    if args.save_strategy == "steps" and args.save_steps < 1:
        raise SystemExit("--save-steps 必须为正数")
    resume_checkpoint = _resolve_resume_checkpoint(args.output, args.resume_from_checkpoint)
    if resume_checkpoint:
        print(f"[checkpoint] 将从完整状态恢复：{resume_checkpoint}")
    elif str(args.resume_from_checkpoint).lower() == "auto":
        print("[checkpoint] 输出目录内没有完整 checkpoint，将开始新训练")
    try:
        guard_blind_final(
