    ...
    with path.open(encoding="utf-8") as f:
        for line in f:
            ...
            try:
                ...
                continue
            if "task_id" in row and not _is_infrastructure_failure(row):
...
def collect_tasks(
    ...
    """Collect resumable tasks, optionally batching independent trajectories.

    Each trajectory remains strictly serial because every shopping action depends
    on the latest observation.  Only independent task attempts run concurrently,
    which lets an OpenAI-compatible serving backend continuously batch requests.
    """
    ...
    if workers > 1 and client_factory is None:
        raise ValueError("client_factory is required when workers > 1")
    ...
    if workers == 1:
        for task, attempt_index in candidates:
    ...
    def submit_available(executor):
        while not infrastructure_failed and len(pending) < workers:
            try:
    ...
    if infrastructure_failed:
        raise CollectionInfrastructureError(
            "collection infrastructure failure; stopped before scheduling more tasks"
        )
    return written
