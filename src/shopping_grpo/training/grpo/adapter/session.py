...
from shopping_grpo.environment.client import ShopAgentEnv, ShopEnvironmentError
...
class ShopSimulatorSession:
    ...
    def __init__(
        ...
        self.reset_retry_delay_seconds = max(0.0, float(reset_retry_delay_seconds))
        self.env = None
        self.state = None
    ...
    async def start(self, task_id: int) -> dict:
        ...
        try:
            ...
            initial = await self._reset_with_capacity_retry(int(task_id))
        except Exception:
            try:
    ...
    async def _reset_with_capacity_retry(self, task_id: int) -> dict:
        """仅对环境池暂满做有界退避；其他 reset 错误立即暴露。"""
        for attempt in range(self.reset_max_attempts):
            try:
                return await asyncio.to_thread(self.env.reset, task_id)
            except ShopEnvironmentError as exc:
                capacity_error = "Unable to get available environment resource" in str(exc)
                if not capacity_error or attempt + 1 >= self.reset_max_attempts:
                    raise
                delay = self.reset_retry_delay_seconds * (2**attempt)
                if delay:
                    await asyncio.sleep(delay)
        raise AssertionError("reset retry loop exhausted without returning or raising")
