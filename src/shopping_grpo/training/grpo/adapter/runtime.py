...
POLICY_FAILURE_TERMINAL_UTILITIES = {
    "assistant_finished_without_environment_done": -0.35,
    "response_budget_exhausted": -0.35,
    "parallel_tool_calls": -0.35,
    "too_many_guard_rejections": -0.65,
    "max_steps": -0.50,
}
...
def make_runtime_state(task_id: int, max_steps: int) -> dict:
    ...
    return {
        ...
        "gigpo_turns": [],
        "gigpo_first_failure_step": None,
        "gigpo_instrumentation_error": None,
    }
...
def reward_breakdown(state: dict) -> dict[str, float | bool]:
    ...
    policy_failure_reason = str(state.get("termination_reason") or "")
    policy_failure = (
        not invalid
        and policy_failure_reason in POLICY_FAILURE_TERMINAL_UTILITIES
    )
    terminal_utility = (
        POLICY_FAILURE_TERMINAL_UTILITIES[policy_failure_reason]
        if policy_failure
        else 0.0
    )
    return {
        ...
        "sampling_invalid": not policy_failure,
        "infrastructure_invalid": invalid or not policy_failure,
        "reward_unverifiable": not policy_failure,
        "policy_failure": policy_failure,
        "policy_failure_reason": policy_failure_reason if policy_failure else "",
    }
