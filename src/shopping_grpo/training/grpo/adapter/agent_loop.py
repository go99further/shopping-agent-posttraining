...
from shopping_grpo.training.grpo.gigpo import (
    latest_assistant_token_span,
    shift_turn_spans,
)
from shopping_grpo.training.grpo.graphgpo import GRAPHGPO_SCHEMA
...
class ShoppingToolAgentLoop(ToolAgentLoop):
    ...
    def __init__(
        ...
        *args,
        ...
        gigpo_process_enable=False,
        gigpo_anchor_mode="structured",
        gigpo_potential_gamma=1.0,
        gigpo_potential_clip=0.25,
        env_factory=None,
        **kwargs,
    ):
        ...
        self.gigpo_process_enable = (
            gigpo_process_enable
            if isinstance(gigpo_process_enable, bool)
            else str(gigpo_process_enable).lower() == "true"
        )
        self.gigpo_anchor_mode = str(gigpo_anchor_mode)
        self.gigpo_potential_gamma = float(gigpo_potential_gamma)
        self.gigpo_potential_clip = float(gigpo_potential_clip)
        ...
        if self.gigpo_anchor_mode not in {"structured", "exact"}:
            raise ValueError("gigpo_anchor_mode must be 'structured' or 'exact'")
        if (
            not math.isfinite(self.gigpo_potential_gamma)
            or not 0.0 < self.gigpo_potential_gamma <= 1.0
        ):
            raise ValueError("gigpo_potential_gamma must be in (0, 1]")
        if not math.isfinite(self.gigpo_potential_clip) or self.gigpo_potential_clip < 0.0:
            raise ValueError("gigpo_potential_clip must be non-negative")
    ...
    async def _handle_generating_state(
        ...
        if stats is not None and stats.removed_tokens:
            ...
            if runtime_state is not None:
                ...
                shift_turn_spans(runtime_state["gigpo_turns"], stats.removed_tokens)
        ...
        if (
            ...
            and not runtime_state.get("done")
            and len(agent_data.response_mask) >= self.response_length
        ):
            runtime_state["terminate"] = True
            runtime_state["termination_reason"] = "response_budget_exhausted"
            runtime_state["error"] = "response_budget_exhausted"
        return next_state
    ...
    async def _call_tool(self, tool_call, tools_kwargs, agent_data):
        ...
        gigpo_turn = None
        if state is not None and getattr(self, "gigpo_process_enable", False):
            try:
                parameters = json.loads(tool_call.arguments or "{}")
                if not isinstance(parameters, dict):
                    parameters = {}
                before_observation = state.get("latest_observation", "")
                before_features = public_state_features(before_observation)
                gigpo_turn = {
                    "step_index": len(state["steps"]),
                    "anchor": canonical_public_anchor(
                        before_observation,
                        mode=getattr(self, "gigpo_anchor_mode", "structured"),
                        features=before_features,
                    ),
                    "exact_anchor": exact_public_observation_hash(
                        before_observation
                    ),
                    "action_key": canonical_action_key(tool_call.name, parameters),
                    "tool_name": str(tool_call.name),
                    "parameters": parameters,
                    "before_observation": before_observation,
                    "token_span": list(
                        latest_assistant_token_span(agent_data.response_mask)
                    ),
                }
            except Exception as exc:
                state["gigpo_instrumentation_error"] = (
                    f"turn_start:{exc.__class__.__name__}:{exc}"
                )
        ...
        if raw_observation is None:
            self._finish_gigpo_turn(state, gigpo_turn)
            return response, reward, step
        try:
    ...
    def _finish_gigpo_turn(self, state, turn):
        """Finalize one public turn after the next visible state is known."""
        if turn is None:
            return
        try:
            verifier = verify_public_transition(
                turn.pop("before_observation"),
                state.get("latest_observation", ""),
                tool_name=turn.pop("tool_name"),
                parameters=turn.pop("parameters"),
                prior_decisions=[
                    (str(previous["anchor"]), str(previous["action_key"]))
                    for previous in state["gigpo_turns"]
                ],
                anchor_mode=getattr(self, "gigpo_anchor_mode", "structured"),
                gamma=getattr(self, "gigpo_potential_gamma", 1.0),
                clip=getattr(self, "gigpo_potential_clip", 0.25),
                terminal=bool(state.get("done")),
                infrastructure_invalid=bool(state.get("infrastructure_invalid")),
            )
            turn["before_potential"] = float(verifier["before_potential"])
            turn["after_potential"] = float(verifier["after_potential"])
            turn["turn_reward"] = float(verifier["process_reward"])
            turn["next_exact_anchor"] = exact_public_observation_hash(
                state.get("latest_observation", "")
            )
            turn["verifier"] = verifier
            state["gigpo_turns"].append(turn)
            if state.get("gigpo_first_failure_step") is None and any(
                bool(value) for value in verifier["failures"].values()
            ):
                state["gigpo_first_failure_step"] = int(turn["step_index"])
        except Exception as exc:
            state["gigpo_instrumentation_error"] = (
                f"turn_finish:{exc.__class__.__name__}:{exc}"
            )
    ...
    async def _handle_processing_tools_state(self, agent_data):
        ...
        if (
            ...
            and runtime_state is not None
            and not runtime_state.get("done")
            and not runtime_state.get("terminate")
        ):
            runtime_state["terminate"] = True
            runtime_state["termination_reason"] = "response_budget_exhausted"
            runtime_state["error"] = "response_budget_exhausted"
    ...
    async def run(self, sampling_params, **kwargs):
        ...
        try:
            ...
            output.extra_fields["shopping"] = {
                ...
                "gigpo": {
                    "enabled": bool(
                        getattr(self, "gigpo_process_enable", False)
                    ),
                    "verifier_schema": PROCESS_VERIFIER_SCHEMA,
                    "graph_schema": GRAPHGPO_SCHEMA,
                    "anchor_mode": getattr(self, "gigpo_anchor_mode", "structured"),
                    "potential_gamma": getattr(self, "gigpo_potential_gamma", 1.0),
                    "potential_clip": getattr(self, "gigpo_potential_clip", 0.25),
                    "anchors": [turn["anchor"] for turn in state["gigpo_turns"]],
                    "exact_anchors": [
                        turn["exact_anchor"] for turn in state["gigpo_turns"]
                    ],
                    "next_exact_anchors": [
                        turn["next_exact_anchor"]
                        for turn in state["gigpo_turns"]
                    ],
                    "action_keys": [
                        turn["action_key"] for turn in state["gigpo_turns"]
                    ],
                    "turn_rewards": [
                        float(turn["turn_reward"]) for turn in state["gigpo_turns"]
                    ],
                    "turn_token_spans": [
                        list(turn["token_span"]) for turn in state["gigpo_turns"]
                    ],
                    "verifier_checks": [
                        dict(turn["verifier"]["checks"])
                        for turn in state["gigpo_turns"]
                    ],
                    "verifier_failures": [
                        dict(turn["verifier"]["failures"])
                        for turn in state["gigpo_turns"]
                    ],
                    "first_failure_step": state["gigpo_first_failure_step"],
                    "instrumentation_error": state["gigpo_instrumentation_error"],
                },
            }
            return output
        finally:
