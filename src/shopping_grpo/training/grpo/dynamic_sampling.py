...
def aggregate_shopping_metrics(shopping_infos: Sequence[object]) -> dict[str, float]:
    ...
    partial_purchase = []
    gigpo_turn_counts = []
    gigpo_unique_anchor_ratios = []
    gigpo_nonzero_reward_ratios = []
    gigpo_instrumentation_errors = []
    gigpo_absolute_reward_sum = 0.0
    gigpo_reward_count = 0
    process_enabled = []
    process_check_totals = {name: 0 for name in PROCESS_CHECK_NAMES}
    process_failure_totals = {name: 0 for name in PROCESS_FAILURE_NAMES}
    process_turn_count = 0
    first_failure_steps = []
    first_failure_trajectories = []
    for index, info in enumerate(shopping_infos):
        ...
        if isinstance(gigpo, Mapping):
            process_enabled.append(float(bool(gigpo.get("enabled"))))
            anchors = list(gigpo.get("anchors") or [])
            raw_turn_rewards = list(gigpo.get("turn_rewards") or [])
            if len(anchors) != len(raw_turn_rewards):
                raise ValueError(
                    f"gigpo diagnostics at index {index} are not aligned"
                )
            absolute_reward_sum = 0.0
            nonzero_reward_count = 0
            for raw_turn_reward in raw_turn_rewards:
                try:
                    turn_reward = float(raw_turn_reward)
                except (TypeError, ValueError) as exc:
                    raise ValueError(
                        f"gigpo turn reward at index {index} is not numeric"
                    ) from exc
                if not math.isfinite(turn_reward):
                    raise ValueError(
                        f"gigpo turn reward at index {index} is not finite"
                    )
                absolute_reward = abs(turn_reward)
                absolute_reward_sum += absolute_reward
                nonzero_reward_count += int(absolute_reward > 1.0e-8)
            turn_count = len(anchors)
            gigpo_turn_counts.append(float(turn_count))
            gigpo_unique_anchor_ratios.append(
                len(set(map(str, anchors))) / max(turn_count, 1)
            )
            gigpo_nonzero_reward_ratios.append(
                nonzero_reward_count / max(turn_count, 1)
            )
            gigpo_absolute_reward_sum += absolute_reward_sum
            gigpo_reward_count += turn_count
            gigpo_instrumentation_errors.append(
                float(bool(gigpo.get("instrumentation_error")))
            )
            raw_checks = list(gigpo.get("verifier_checks") or [])
            raw_failures = list(gigpo.get("verifier_failures") or [])
            if raw_checks or raw_failures:
                if not (
                    len(raw_checks) == len(raw_failures) == turn_count
                ):
                    raise ValueError(
                        f"process verifier diagnostics at index {index} are not aligned"
                    )
                for turn_index, (checks, failures) in enumerate(
                    zip(raw_checks, raw_failures, strict=True)
                ):
                    if not isinstance(checks, Mapping) or not isinstance(
                        failures, Mapping
                    ):
                        raise ValueError(
                            "process verifier turn at index "
                            f"{index}, turn {turn_index} is not an object"
                        )
                    for name in PROCESS_CHECK_NAMES:
                        process_check_totals[name] += int(bool(checks.get(name)))
                    for name in PROCESS_FAILURE_NAMES:
                        process_failure_totals[name] += int(
                            bool(failures.get(name))
                        )
                process_turn_count += turn_count
                first_failure = gigpo.get("first_failure_step")
                first_failure_trajectories.append(float(first_failure is not None))
                if first_failure is not None:
                    first_failure = int(first_failure)
                    if first_failure < 0:
                        raise ValueError(
                            f"process first_failure_step at index {index} is invalid"
                        )
                    first_failure_steps.append(float(first_failure))
    ...
    metrics = {
        ...
        "gigpo/turns_mean": mean(gigpo_turn_counts) if gigpo_turn_counts else 0.0,
        "gigpo/unique_anchor_ratio": (
            mean(gigpo_unique_anchor_ratios) if gigpo_unique_anchor_ratios else 0.0
        ),
        "gigpo/nonzero_turn_reward_ratio": (
            mean(gigpo_nonzero_reward_ratios) if gigpo_nonzero_reward_ratios else 0.0
        ),
        "gigpo/absolute_turn_reward_mean": (
            gigpo_absolute_reward_sum / gigpo_reward_count
            if gigpo_reward_count
            else 0.0
        ),
        "gigpo/instrumentation_error_rate": (
            mean(gigpo_instrumentation_errors) if gigpo_instrumentation_errors else 0.0
        ),
        "process_verifier/enabled_rate": (
            mean(process_enabled) if process_enabled else 0.0
        ),
        "process_verifier/first_failure_trajectory_rate": (
            mean(first_failure_trajectories)
            if first_failure_trajectories
            else 0.0
        ),
        "process_verifier/first_failure_step_mean": (
            mean(first_failure_steps) if first_failure_steps else 0.0
        ),
    }
    for name, total in process_check_totals.items():
        metrics[f"process_verifier/check_{name}_rate"] = (
            total / process_turn_count if process_turn_count else 0.0
        )
    for name, total in process_failure_totals.items():
        metrics[f"process_verifier/failure_{name}_rate"] = (
            total / process_turn_count if process_turn_count else 0.0
        )
    return metrics
...
def select_reward_varying_groups(
    ...
    *,
    ...
    allow_constant_success: bool = False,
    ...
    for uid, group in grouped.items():
        ...
        all_purchase_success = all(group["purchase_success"])
        reasons = tuple(sorted(set(group["sampling_invalid_reasons"])))
        if has_sampling_invalid:
            drop_reason = "sampling_invalid"
        elif not utility_varying and not (
            allow_constant_success and all_purchase_success
        ):
        ...
        groups.append(
            {
                ...
                "confidence_only": bool(
                    allow_constant_success
                    and all_purchase_success
    ...
    stats = {
        ...
        "confidence_only_group_count": sum(
            group["confidence_only"] for group in groups
        ),
