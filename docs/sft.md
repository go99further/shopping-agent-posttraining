...
| Checkpoint interval | every 10 optimizer steps |
| Retained checkpoints | latest 3 |
| Resume policy | latest complete checkpoint in the output directory |
...
The trainer resumes automatically when the adapter output directory contains a
complete `checkpoint-*` directory. A resumable checkpoint must contain the LoRA
weights plus Trainer, optimizer, scheduler and RNG state; partially written
directories are skipped. Pass `--resume-from-checkpoint none` to deliberately
start a fresh run in an empty output directory, or pass an explicit checkpoint
path to require that exact checkpoint.
